File comes from GMR/assets/unitree_g1
...
- IsaacLab training (URDF): use `g1.urdf`.
- MuJoCo HOI with articulated hands: use `g1_with_hands.xml`.
- MuJoCo baseline without finger articulation: use `g1_nohands.xml`.

Note:
`g1.urdf` currently uses fixed rubber-hand links (no articulated fingers).
If articulated hand URDF is required in IsaacLab, attach a hand model from `assets/hands/*` and define explicit mount joints.
- IsaacLab with Inspire hands: use `g1_inspire_hand.urdf`.
